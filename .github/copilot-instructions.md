...
> **Version:** 9.0.0 | **Status:** Production | **PHP 8.3+** | **Session-based** | **AJAX-driven**
...
### 3. Centralized Payment API (v9.0)
- **Hub:** [config/api/checkout.php](config/api/checkout.php) — all payment paths converge here
- **Handles:**
...
| Task | Command/Path |
|------|--------------|
| Add new product | Edit [admin/config.php](admin/config.php#L1-L20) pricing array |
...
4. **Payment retry:** Decline → retry modal → session restoration
5. **On-screen keyboard:** Forms shift 240px up; submit visible on 1080p displays
...
1. [shopping_cart.class.php](shopping_cart.class.php) — Core pricing & session model
2. [public/assets/js/acps_modal.js](public/assets/js/acps_modal.js) — Modal architecture
3. [config/api/checkout.php](config/api/checkout.php) — Payment centralization
4. [pay.php](pay.php) — Checkout flow & retry logic
5. [admin/config.php](admin/config.php) — Pricing config + location setup
